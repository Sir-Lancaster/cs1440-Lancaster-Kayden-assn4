class fractalinformation:
    images = {
            'type': 'mandelbrot', 'mandelbrot': {
                'centerX': -0.6,
                'centerY': 0.0,
                'axisLen': 2.5,
                },

            'type': 'mandelbrot-zoomed', 'mandelbrot-zoomed': {
                'centerX': -1.0,
                'centerY': 0.0,
                'axisLen': 1.0,
                },

            'type': 'spiral0', 'spiral0': {
                'centerX': -0.761335372924805,
                'centerY': 0.0835704803466797,
                'axisLen': 0.004978179931102462,
                },

            'type': 'spiral1', 'spiral1': {
                'centerX': -0.747,
                'centerY': 0.1075,
                'axisLen': 0.002,
                },

            'type': 'seahorse', 'seahorse': {
                'centerX': -0.748,
                'centerY': -0.102,
                'axisLen': 0.008,
                },

            'type': 'elephants', 'elephants': {
                'centerX':  0.3015,
                'centerY':  -0.0200,
                'axisLen':  0.03,
                },

            'type': 'leaf', 'leaf': {
                'centerX': -1.543577002,
                'centerY': -0.000058690069,
                'axisLen':  0.000051248888,
                },

            'type': 'starfish', 'starfish': {
                'centerX': -0.463595023481762,
                'centerY': 0.598380871135558,
                'axisLen': 0.00128413675654471,
                },

            'type': 'phoenix', 'phoenix': {
                'centerX':     0.0,
                'centerY':     0.0,
                'axisLen':  3.25,
                },

            'type': 'peacock', 'peacock': {
                'centerX':     -0.363287878200906,
                'centerY':     0.381197981824009,
                'axisLen':  0.0840187115019564,
            },

            'type': 'monkey-knife-fight', 'monkey-knife-fight': {
                'centerX':    -0.945542168674699,
                'centerY':    0.232234726688103,
                'axisLen': 0.136626506024096,
                },

            'type': 'shrimp-cocktail', 'shrimp-cocktail': {
                'centerX': 0.529156626506024,
                'centerY': -0.3516077170418,
                'axisLen': 0.221204819277108,
                },
            }